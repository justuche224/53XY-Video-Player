...
| Library scan misses new files | New videos present in MediaStore never ingested by `scanVideos` (repro: pushed mp4s on phone; user-seen on emulator). Pre-existing, NOT from feel pack. Default-limit theory disproven against native Query source | 🐛 bug — needs own debugging session |
...
Tests: **303 passing**, `npx tsc --noEmit` clean.
...
In flight: the **player feel pack** ([spec](./superpowers/specs/2026-07-28-player-feel-pack-design.md)) — features A (aspect modes + pinch-zoom) and B (binge pack) are merged; feature C (scrub preview thumbnails) remains. Also queued: the **library-scan-misses-new-files bug** (row above). Product calls accepted in B, revisitable: auto-advance honors saved resume positions; minutes sleep-timer survives next/prev; countdown card is text-only (no ring). Other candidates from the backlog rows above:
