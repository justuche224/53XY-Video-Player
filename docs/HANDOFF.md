...
| Player: gesture-wedge root fix | `surfaceType="textureView"` + `pointerEvents="none"` on VideoView — video view removed from touch dispatch and window-surface lifecycle | ⏳ device-verify pending |
...
- **Player gesture-wedge root fix** — the recurring "player goes unresponsive" wedge (rotation, then pause / subtitle-select) is attacked at its class root instead of per-trigger remounts: the native video view is removed from BOTH touch dispatch and window-surface lifecycle. `VideoView` now renders a TextureView (`surfaceType="textureView"` — ordinary composited view, no separate window surface; DRM-only limitation irrelevant for local files) inside a `pointerEvents="none"` wrapper (expo-video's Android view otherwise consumes every touch and re-dispatches synthetic events into RN, expo/expo#35479 — it must never be a touch target under our RNGH layer). Prior workarounds (`gestureGen` remounts, `unstable_forceActive` root) kept as belt-and-braces; strip them in a follow-up once all triggers verify clean on device. If pause still wedges after this, the next lever is `player.keepScreenOnWhilePlaying = false` (redundant with the screen-level `useKeepAwake()`, and its play/pause `keepScreenOn` window-flag flip was the suspected pause trigger). JS-only.
