...
Tests: **292 passing**, `npx tsc --noEmit` clean.
...
In flight: the **player feel pack** ([spec](./superpowers/specs/2026-07-28-player-feel-pack-design.md)) — feature A (aspect modes + pinch-zoom) is merged; features B (autoplay-next countdown + sleep timer) and C (scrub preview thumbnails) are specced, awaiting their implementation plans. Also queued: the **library-scan-misses-new-files bug** (row above) — investigate before or alongside feature B. Other candidates from the backlog rows above:
