...
_Last updated: after the first v2 feature — the **library length filter** — was built, reviewed, and merged to `master`. **Device-verify still pending** on the SM-S901N (JS-only; `npx expo start`, no rebuild)._
...
| v2 — Length filter | Persistent min/max duration ignore rules; Settings UI (chips + custom dialog + hidden-count footer); applied in useLibrary + useGroups | ✅ merged to `master`; ⏳ device-verify pending |
