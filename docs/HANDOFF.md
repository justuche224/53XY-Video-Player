...
_Last updated: after v2 **advanced filters** (length + name-pattern + folder ignore), **library sort options**, and the **FlashList migration** were built, reviewed, merged to `master`, and device-verified on the SM-S901N. (libVLC fallback is parked — too big/native to do blind.)_
