...
| Bottom tab bar | Custom floating Material You tab bar (Home/Playlists/History/Settings); `(tabs)` group | ✅ merged (device-verify pending) |
...
- **Bottom tab bar** — top nav-icon row replaced by a 4-tab bottom bar (Home/Playlists/History/Settings) via an Expo Router `(tabs)` group; custom floating pill `tabBar` in Material You colors, spring-animated active icon lifting into a `primary` circle. Detail screens stay full-screen; Home header keeps Sort+Layout only; Resume FAB raised above the bar. Pure `tabIconFor` Jest-tested. JS-only. Spec: [bottom-tab-bar-design](./superpowers/specs/2026-06-23-bottom-tab-bar-design.md).
