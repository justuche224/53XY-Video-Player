...
- **UI Polish merged:** Unified icons (`Ionicons` and `MaterialIcons`) replacing placeholder emojis/text across the home screen, player chrome, and settings. Applied properly themed headers to `GroupDetailScreen` and `SettingsScreen` to fix the stark white defaults in dark mode. Added `bounces={true}` and `overScrollMode="always"` to all list views to fix scroll stiffness. Spec/plan: [superpowers/specs/2026-06-18-ui-polish-design.md](./superpowers/specs/2026-06-18-ui-polish-design.md), [superpowers/plans/2026-06-18-ui-polish.md](./superpowers/plans/2026-06-18-ui-polish.md).
...
| v2 — UI Polish | Unified Ionicons/MaterialIcons across the app, themed headers for Settings/Group, scroll overscroll/bounces for all lists | ✅ merged, device-verified |
