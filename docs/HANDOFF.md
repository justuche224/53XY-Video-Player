...
- **Next: v2 backlog** (nothing in flight). Candidates: advanced filters (ignore by length/name, folder-only), libVLC fallback for exotic codecs, playlists, FlashList for thumbnail fling perf, and the two deferred player refinements below.
...
| **v1 complete** | Foundation + Library + Player all shipped | 🎉 — next work is v2 backlog |
